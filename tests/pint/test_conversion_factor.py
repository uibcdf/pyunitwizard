...
def test_conversion_factor_is_cached():
    ...
    assert not any(k[0] == "nm" and k[1] == "angstroms" for k in kernel.conversion_factor_cache)
    first = puw.conversion_factor("nm", "angstroms")
    assert any(k[0] == "nm" and k[1] == "angstroms" for k in kernel.conversion_factor_cache)
