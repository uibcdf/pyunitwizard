...
from .comparison import _compatible_dimensionalities, are_equal
from .extraction import get_unit, get_value
from .introspection import get_dimensionality, has_unit, is_quantity, is_unit
...
def check(
    ...
    if is_quantity(quantity_or_unit):
        if unit is not None:
            unit_matches = has_unit(quantity_or_unit, unit)
            if unit_matches is False:
                return False
            if unit_matches is None and not are_equal(get_unit(quantity_or_unit), unit):
                return False
        ...
        if unit is not None:
            ...
            if unit_matches is False:
                return False
            if unit_matches is None and not are_equal(quantity_or_unit, unit):
                return False
        if dimensionality is not None:
            aux_dimensionality = get_dimensionality(quantity_or_unit)
            if not _compatible_dimensionalities(aux_dimensionality, dimensionality):
