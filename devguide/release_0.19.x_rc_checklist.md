...
## Evidence log (2026-03-04)

- Local quality baseline: `pytest -q tests --ignore=tests/test_import.py` -> `356 passed` on Python `3.13`.
- RC checkpoint tag published: `0.19.3`.
- Integration evidence:
  ...
  - `tests/utils/numpy/test_transparent_bridge.py` and `tests/utils/numpy/test_ops.py` (transparent + explicit NumPy operations coverage).
  - `tests/utils/pandas/test_frame_contract.py` and `tests/utils/pandas/test_workflows.py` (DataFrame boundary and metadata-preservation contracts).
  - `tests/utils/matplotlib/test_units_bridge.py` and `tests/utils/matplotlib/test_complex_layouts.py` (bridge behavior and advanced layout compatibility).
  - `tests/integration/test_frontend_cross_backend_matrix.py` (cross-backend frontend matrix over NumPy/Pandas/Matplotlib).
